...
public:
    ...
    // Enqueue-only library boundary. Telemetry is requested separately so the
    // frame hot path never performs a host readback or waits for CUDA events.
    void finish_frame_async(cudaStream_t stream = nullptr);
    void collect_telemetry_async(cudaStream_t stream = nullptr);
