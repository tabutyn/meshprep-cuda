...
[[nodiscard]] float4 normalize(float4 value) noexcept
{
    ...
    return magnitude > 1.0e-8F
        ? float4{value.x/magnitude, value.y/magnitude,
...
[[nodiscard]] bool valid(RigidBodyOptions options) noexcept
{
    const bool shape_valid = options.shape == RigidShape::sphere
        ...
        : finite(options.half_extents) && options.half_extents.x > 0.0F &&
            options.half_extents.y > 0.0F && options.half_extents.z > 0.0F;
    return shape_valid && finite(options.mass) && options.mass > 0.0F &&
        finite(options.inertia) && options.inertia.x > 0.0F &&
        options.inertia.y > 0.0F && options.inertia.z > 0.0F &&
...
Status RigidBody::create(
    RigidBodyState state, RigidBodyOptions options, RigidBody& output) noexcept
{
    return output.initialize(state, options);
}

Status RigidBody::initialize(
    RigidBodyState state, RigidBodyOptions options) noexcept
{
    if (!valid(options) || !finite(state.position) ||
        !finite(state.orientation) || !finite(state.linear_velocity) ||
        !finite(state.angular_velocity)) return invalid("invalid rigid-body state");
...
Status RigidBody::prepare_substep(SubstepContext value, cudaStream_t) noexcept
{
    ...
    if (impl_->substep_prepared || value.index != impl_->next_substep ||
        value.count != impl_->frame.substeps || value.timestep <= 0.0F ||
        !finite(value.timestep) || !finite(value.acceleration)) {
...
{
    if (!impl_ || !impl_->substep_prepared || !finite(force) ||
        !finite(world_point)) return invalid("rigid-body force phase is not active");
    impl_->force = add(impl_->force, force);
    impl_->torque = add(impl_->torque,
        cross(subtract(world_point, impl_->state.position), force));
    return {};
}
...
Status RigidBody::finish_substep(SubstepContext value, cudaStream_t) noexcept
{
    if (!impl_ || !impl_->frame_active || !impl_->substep_prepared ||
        value.index != impl_->next_substep ||
        value.count != impl_->frame.substeps) {
    ...
    impl_->state.linear_velocity = add(impl_->state.linear_velocity,
        multiply(add(value.acceleration, multiply(impl_->force, inverse_mass)), dt));
    impl_->state.linear_velocity = multiply(impl_->state.linear_velocity,
        std::exp(-impl_->options.linear_damping*dt));
    impl_->state.linear_velocity = clamp_length(impl_->state.linear_velocity,
        impl_->options.maximum_linear_speed);
    impl_->state.position = add(impl_->state.position,
        multiply(impl_->state.linear_velocity, dt));

    const float3 angular_acceleration{
        impl_->torque.x/impl_->options.inertia.x,
        impl_->torque.y/impl_->options.inertia.y,
        impl_->torque.z/impl_->options.inertia.z};
    impl_->state.angular_velocity = add(impl_->state.angular_velocity,
        multiply(angular_acceleration, dt));
    impl_->state.angular_velocity = multiply(impl_->state.angular_velocity,
        std::exp(-impl_->options.angular_damping*dt));
    impl_->state.angular_velocity = clamp_length(impl_->state.angular_velocity,
        impl_->options.maximum_angular_speed);
    const float4 spin = quaternion_product(
        float4{impl_->state.angular_velocity.x,
            impl_->state.angular_velocity.y,
            impl_->state.angular_velocity.z, 0.0F},
        impl_->state.orientation);
    impl_->state.orientation = normalize(float4{
        impl_->state.orientation.x + 0.5F*dt*spin.x,
        impl_->state.orientation.y + 0.5F*dt*spin.y,
        impl_->state.orientation.z + 0.5F*dt*spin.z,
        impl_->state.orientation.w + 0.5F*dt*spin.w});
    if (!finite(impl_->state.position) ||
        !finite(impl_->state.orientation) ||
        !finite(impl_->state.linear_velocity) ||
        !finite(impl_->state.angular_velocity)) {
...
Status RigidBody::advance_async(
...
RigidBodyState RigidBody::state() const noexcept
{
    return impl_ ? impl_->state : RigidBodyState{};
}
