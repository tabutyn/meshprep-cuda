...
{
    return error == cudaSuccess ? Status{} : Status{
...
bool valid(const Collider& value)
{
    ...
    case ColliderShape::box:
        valid_dimensions = value.dimensions.x > 0.0F &&
            value.dimensions.y > 0.0F && value.dimensions.z > 0.0F;
        break;
    ...
    case ColliderShape::capsule:
        valid_dimensions = value.dimensions.x > 0.0F &&
            value.dimensions.y >= 0.0F;
        break;
    default:
        return false;
    }
    return finite(value.position) && finite(value.orientation) &&
        finite(orientation_length_squared) &&
        orientation_length_squared > 1.0e-12F &&
        finite(value.linear_velocity) && finite(value.angular_velocity) &&
        finite(value.dimensions) && value.dimensions.x >= 0.0F &&
        value.dimensions.y >= 0.0F && value.dimensions.z >= 0.0F &&
        valid_dimensions &&
        finite(value.friction) && value.friction >= 0.0F &&
        finite(value.restitution) && value.restitution >= 0.0F &&
        finite(value.contact_offset) && value.contact_offset >= 0.0F;
}

__global__ void prepare_records(const ConstraintRecord* input,
    std::uint64_t* keys, ConstraintRecord* values, std::uint32_t count)
{
...
__device__ std::uint32_t lower_bound_point(
    const std::uint64_t* keys, std::uint32_t count, std::uint32_t point)
{
...
__global__ void gather_records(const std::uint64_t* keys,
    const ConstraintRecord* records, std::uint32_t record_count,
    PointCouplingView target)
{
...
Status ColliderSet::reserve(std::uint32_t capacity) noexcept
{
    ...
    try {
        ...
        Status status = cuda_status(cudaMalloc(&replacement->data,
            static_cast<std::size_t>(capacity) * sizeof(Collider)),
...
Status ColliderSet::update_async(
    std::span<const Collider> colliders, cudaStream_t stream) noexcept
{
    ...
    Status status = cuda_status(cudaMemcpyAsync(impl_->data, colliders.data(),
        colliders.size_bytes(), cudaMemcpyHostToDevice, stream),
        "could not upload colliders");
    if (status) impl_->size = static_cast<std::uint32_t>(colliders.size());
    return status;
}

Status ColliderSet::update(
    std::span<const Collider> colliders, cudaStream_t stream) noexcept
{
    Status status = update_async(colliders, stream);
    return status ? cuda_status(cudaStreamSynchronize(stream),
                        "could not complete collider upload") : status;
}
...
std::uint32_t ColliderSet::capacity() const noexcept
{
...
Status ConstraintBatch::reserve(
    std::uint32_t capacity, cudaStream_t stream) noexcept
{
    ...
    try {
        ...
        if (!(status = cuda_status(cudaMalloc(&replacement->keys_b, key_bytes),
                "could not allocate sorted constraint keys"))) return status;
        if (!(status = cuda_status(cudaMalloc(&replacement->records_a, value_bytes),
                "could not allocate constraint records"))) return status;
        if (!(status = cuda_status(cudaMalloc(&replacement->records_b, value_bytes),
                "could not allocate sorted constraint records"))) return status;
        cudaError_t error = cub::DeviceRadixSort::SortPairs(nullptr,
            replacement->sort_storage_bytes, replacement->keys_a,
            replacement->keys_b, replacement->records_a,
            replacement->records_b, static_cast<int>(capacity), 0, 64, stream);
        if (!(status = cuda_status(error,
                "could not size constraint sort workspace"))) return status;
        if (!(status = cuda_status(cudaMalloc(&replacement->sort_storage,
                replacement->sort_storage_bytes),
                "could not allocate constraint sort workspace"))) return status;
        replacement->capacity = capacity;
        impl_ = std::move(replacement);
        return {};
    } catch (const std::bad_alloc&) {
...
Status ConstraintBatch::apply_async(ConstraintRecordView records,
    PointCouplingView target, cudaStream_t stream) noexcept
{
    if (records.count == 0U) return {};
    if (records.data == nullptr || target.count == 0U ||
        target.positions == nullptr || target.velocities == nullptr ||
        (target.external_impulses == nullptr &&
            target.position_corrections == nullptr)) {
    ...
    const std::uint32_t record_blocks =
        (records.count + block_size - 1U) / block_size;
    prepare_records<<<record_blocks, block_size, 0, stream>>>(records.data,
        impl_->keys_a, impl_->records_a, records.count);
    Status status = cuda_status(cudaGetLastError(),
        "could not prepare deterministic constraints");
    if (!status) return status;
    status = cuda_status(cub::DeviceRadixSort::SortPairs(impl_->sort_storage,
        impl_->sort_storage_bytes, impl_->keys_a, impl_->keys_b,
        impl_->records_a, impl_->records_b, static_cast<int>(records.count),
        0, 64, stream), "could not sort deterministic constraints");
    if (!status) return status;
    gather_records<<<(target.count + block_size - 1U) / block_size,
        block_size, 0, stream>>>(impl_->keys_b, impl_->records_b,
        records.count, target);
    return cuda_status(cudaGetLastError(),
        "could not gather deterministic constraints");
}

Status ConstraintBatch::apply(ConstraintRecordView records,
    PointCouplingView target, cudaStream_t stream) noexcept
{
    Status status = apply_async(records, target, stream);
    return status ? cuda_status(cudaStreamSynchronize(stream),
                        "could not complete deterministic constraints") : status;
}

std::uint32_t ConstraintBatch::capacity() const noexcept
{
    return impl_ ? impl_->capacity : 0U;
}
...
std::size_t ConstraintBatch::allocated_bytes() const noexcept
{
    return impl_ ? impl_->sort_storage_bytes +
        static_cast<std::size_t>(impl_->capacity) *
            (2U * sizeof(std::uint64_t) + 2U * sizeof(ConstraintRecord)) : 0U;
}
