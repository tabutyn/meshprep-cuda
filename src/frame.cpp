...
{
    return std::isfinite(options.timestep) && options.timestep > 0.0F &&
        options.substeps > 0U && options.substeps <= 256U &&
        finite(options.acceleration);
}

SubstepContext substep_context(FrameOptions options, std::uint32_t index) noexcept
{
    return {index, options.substeps,
...
Status Completion::record(cudaStream_t stream) noexcept
{
    ...
    status_ = cuda_status(
        cudaEventRecord(event_, stream), "could not record completion event");
    pending_ = status_.ok();
    return status_;
}
...
Status Completion::wait() noexcept
{
    ...
    status_ = cuda_status(
        cudaEventSynchronize(event_), "asynchronous CUDA operation failed");
    pending_ = false;
    return status_;
}
