...
struct MergeSegmentStats {
    __host__ __device__ SegmentStats operator()(const SegmentStats &a,
                                                const SegmentStats &b) const {
        return {
...
template <typename T> void release(T *&pointer) noexcept {
...
__device__ Vec3 subtract(Vec3 a, Vec3 b) { return {a.x - b.x, a.y - b.y, a.z - b.z}; }
...
__global__ void validate_positions_kernel(const float3 *positions, std::uint32_t count,
                                          std::uint32_t *flags) {
...
__global__ void validate_triangles_kernel(const uint3 *triangles, std::uint32_t count,
                                          std::uint32_t vertex_count, std::uint32_t *flags) {
    const std::uint32_t index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index >= count) return;
...
__global__ void validate_edges_kernel(const uint2 *edges, std::uint32_t count,
                                      std::uint32_t vertex_count, std::uint32_t *flags) {
    const std::uint32_t index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index >= count) return;
...
[[nodiscard]] Status validate_mesh(DeviceMeshView mesh, SharpEdgeView edges, std::uint32_t *flags,
                                   cudaStream_t stream) {
    ...
    validate_positions_kernel<<<(vertex_count + block_size - 1) / block_size, block_size, 0,
                                stream>>>(mesh.positions, vertex_count, flags);
    validate_triangles_kernel<<<(triangle_count + block_size - 1) / block_size, block_size, 0,
                                stream>>>(mesh.triangles, triangle_count, vertex_count, flags);
    if (edge_count > 0) {
        validate_edges_kernel<<<(edge_count + block_size - 1) / block_size, block_size, 0,
                                stream>>>(edges.edges, edge_count, vertex_count, flags);
    }
...
__global__ void face_normals_kernel(const float3 *positions, const uint3 *triangles,
                                    float3 *face_normals, Vec3 *area_normals, std::uint32_t count,
...
__global__ void make_corner_records_kernel(const uint3 *triangles, std::uint64_t *keys,
                                           std::uint32_t *corners, std::uint32_t triangle_count) {
    ...
    for (std::uint32_t corner = 0; corner < 3; ++corner) {
        ...
        keys[global_corner] = (static_cast<std::uint64_t>(vertices[corner]) << 32U) | global_corner;
        corners[global_corner] = global_corner;
    }
}

__global__ void make_edge_records_kernel(const uint2 *edges, std::uint64_t *keys,
...
__global__ void count_vertices_from_corner_keys_kernel(const std::uint64_t *keys,
                                                       std::uint32_t count,
...
__global__ void count_vertices_from_edge_keys_kernel(const std::uint64_t *keys, std::uint32_t count,
                                                     std::uint32_t *vertex_counts) {
    const std::uint32_t index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index >= count) return;
    atomicAdd(vertex_counts + static_cast<std::uint32_t>(keys[index] >> 32U), 1U);
...
__device__ bool contains_edge(const std::uint64_t *edge_keys, std::uint32_t begin,
                              std::uint32_t end, std::uint64_t wanted) {
    const std::uint32_t original_end = end;
    while (begin < end) {
        const std::uint32_t middle = begin + (end - begin) / 2U;
        if (edge_keys[middle] < wanted)
            begin = middle + 1U;
        else
            end = middle;
    }
...
__device__ std::uint32_t find_root(std::uint32_t *labels, std::uint32_t base, std::uint32_t value) {
    ...
    while (labels[base + root] != root)
        root = labels[base + root];
...
__device__ void other_vertices(uint3 triangle, std::uint32_t corner, std::uint32_t &first,
                               std::uint32_t &second) {
    if (corner == 0U) {
        first = triangle.y;
        second = triangle.z;
...
__global__ void group_vertex_fans_kernel(const uint3 *triangles,
                                         const std::uint32_t *sorted_corners,
                                         const std::uint32_t *corner_offsets,
                                         const std::uint64_t *sorted_edge_keys,
                                         const std::uint32_t *edge_offsets, std::uint32_t *labels,
                                         std::uint32_t *group_counts, std::uint32_t vertex_count) {
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
        ...
        for (std::uint32_t j = i + 1U; j < count; ++j) {
            ...
            const std::uint64_t edge_key = (static_cast<std::uint64_t>(vertex) << 32U) | shared;
...
__global__ void
emit_vertex_normals_kernel(const std::uint32_t *sorted_corners, const std::uint32_t *corner_offsets,
                           const std::uint32_t *labels, const std::uint32_t *group_offsets,
                           const Vec3 *area_normals, float3 *vertex_normals,
                           std::uint32_t *corner_normal_indices, std::uint32_t vertex_count) {
    const std::uint32_t vertex = blockIdx.x * blockDim.x + threadIdx.x;
    if (vertex >= vertex_count) return;
    const std::uint32_t begin = corner_offsets[vertex];
...
__global__ void hierarchy_geometry_kernel(const float3 *positions, const uint3 *triangles,
                                          Vec3 *centroids, Bounds *triangle_bounds,
                                          std::uint32_t count) {
    const std::uint32_t index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index >= count) return;
    const uint3 triangle = triangles[index];
...
__global__ void aabb_proxy_kernel(const Aabb *bounds, float3 *positions, uint3 *triangles,
                                  std::uint32_t count, std::uint32_t *invalid) {
    const std::uint32_t index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index >= count) return;
    ...
    positions[base + 2U] =
        make_float3(0.5F * (box.minimum.x + box.maximum.x), 0.5F * (box.minimum.y + box.maximum.y),
...
__global__ void gather_segment_stats_kernel(const Vec3 *centroids,
                                            const std::uint32_t *primitive_indices,
                                            SegmentStats *ordered, std::uint32_t count) {
    const std::uint32_t index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index >= count) return;
...
__global__ void make_partition_keys_kernel(const Vec3 *centroids,
                                           const std::uint32_t *primitive_indices,
                                           const std::uint32_t *segment_ids,
                                           const std::uint32_t *segment_offsets,
                                           const SegmentStats *stats, std::uint64_t *keys,
                                           std::uint32_t count, bool force_balanced) {
    const std::uint32_t index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index >= count) return;
    ...
    const std::uint32_t balanced_bucket = min(
        7U, static_cast<std::uint32_t>((static_cast<std::uint64_t>(local) * 8U) / segment_count));
    const Vec3 centroid = centroids[primitive_indices[index]];
    ...
    const std::uint32_t x = (force_balanced || flat_x)
                                ? (balanced_bucket & 1U)
                                : static_cast<std::uint32_t>(centroid.x > mean.x);
    const std::uint32_t y = (force_balanced || flat_y)
                                ? ((balanced_bucket >> 1U) & 1U)
                                : static_cast<std::uint32_t>(centroid.y > mean.y);
    const std::uint32_t z = (force_balanced || flat_z)
                                ? ((balanced_bucket >> 2U) & 1U)
...
__global__ void classify_children_kernel(const std::uint32_t *child_counts,
                                         std::uint32_t *branch_flags, std::uint32_t *branch_weights,
                                         std::uint32_t *leaf_weights, std::uint32_t child_count,
...
__global__ void collect_totals_kernel(const std::uint32_t *branch_flags,
                                      ...
                                      const std::uint32_t *leaf_weight_prefix, std::uint32_t count,
                                      std::uint32_t *totals) {
...
__global__ void count_parent_children_kernel(const std::uint64_t *unique_keys,
...
__global__ void set_parent_nodes_kernel(const std::uint32_t *parent_node_ids,
                                        const std::uint32_t *parent_counts,
                                        const std::uint32_t *parent_prefix, HierarchyNode *nodes,
                                        std::uint32_t parent_count, std::uint32_t child_node_base) {
...
__global__ void initialize_child_nodes_kernel(
    const std::uint32_t *child_counts, const std::uint32_t *branch_flags,
    const std::uint32_t *branch_prefix, const std::uint32_t *branch_weight_prefix,
    const std::uint32_t *leaf_weight_prefix, std::uint32_t *next_offsets,
    std::uint32_t *next_node_ids, std::uint32_t *all_branch_node_ids, HierarchyNode *nodes,
    std::uint32_t child_count, std::uint32_t child_node_base, std::uint32_t branch_list_base,
...
__global__ void scatter_child_primitives_kernel(
    const std::uint32_t *child_offsets, const std::uint32_t *child_counts,
    const std::uint32_t *branch_flags, const std::uint32_t *branch_prefix,
    const std::uint32_t *branch_weight_prefix, const std::uint32_t *leaf_weight_prefix,
    const std::uint32_t *sorted_primitives, std::uint32_t *next_primitives,
    std::uint32_t *next_segment_ids, std::uint32_t *final_primitives, std::uint32_t child_count,
    std::uint32_t leaf_primitive_base) {
...
__global__ void leaf_bounds_kernel(const std::uint32_t *child_offsets,
                                   const std::uint32_t *child_counts,
                                   const std::uint32_t *branch_flags,
                                   const std::uint32_t *sorted_primitives,
                                   const Bounds *triangle_bounds, HierarchyNode *nodes,
                                   std::uint32_t child_count, std::uint32_t child_node_base) {
    const std::uint32_t child = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void root_leaf_kernel(const Bounds *triangle_bounds, HierarchyNode *nodes,
                                 std::uint32_t count) {
    if (threadIdx.x != 0 || blockIdx.x != 0) return;
    Vec3 minimum{INFINITY, INFINITY, INFINITY};
    Vec3 maximum{-INFINITY, -INFINITY, -INFINITY};
...
__global__ void branch_bounds_kernel(const std::uint32_t *branch_node_ids, HierarchyNode *nodes,
                                     std::uint32_t count) {
    const std::uint32_t index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index >= count) return;
...
__global__ void validate_aabb_kernel(const Aabb *bounds, std::uint32_t count,
    ...
    const bool finite = isfinite(box.minimum.x) && isfinite(box.minimum.y) &&
                        isfinite(box.minimum.z) && isfinite(box.maximum.x) &&
                        isfinite(box.maximum.y) && isfinite(box.maximum.z);
    const bool ordered = box.minimum.x <= box.maximum.x && box.minimum.y <= box.maximum.y &&
                         box.minimum.z <= box.maximum.z;
    if (!finite || !ordered) atomicAdd(invalid_count, 1U);
}

__global__ void refit_leaf_bounds_kernel(const Aabb *primitive_bounds,
                                         const std::uint32_t *primitive_indices,
                                         HierarchyNode *nodes, std::uint32_t node_count) {
...
const char *status_code_name(StatusCode code) noexcept {
    ...
    case StatusCode::invalid_mesh:
        return "invalid_mesh";
    case StatusCode::unsupported_size:
        return "unsupported_size";
    case StatusCode::allocation_failure:
        return "allocation_failure";
    case StatusCode::cuda_failure:
        return "cuda_failure";
...
Status compute_normals(DeviceMeshView mesh, SharpEdgeView sharp_edges, Workspace &workspace,
    ...
    if (!status) return status;
    status =
        ensure_allocation(output.corner_normal_indices_, output.corner_capacity_, corner_count);
    if (!status) return status;
    ...
    status = cub_query(
        ...
        [&](void *temporary, std::size_t &bytes) {
            return cub::DeviceRadixSort::SortPairs(
                temporary, bytes, static_cast<std::uint64_t *>(nullptr),
                static_cast<std::uint64_t *>(nullptr), static_cast<std::uint32_t *>(nullptr),
                static_cast<std::uint32_t *>(nullptr), corner_count, 0, 64, stream);
        },
    ...
    status = cub_query(
        ...
        [&](void *temporary, std::size_t &bytes) {
            return cub::DeviceScan::ExclusiveSum(
                temporary, bytes, static_cast<std::uint32_t *>(nullptr),
                static_cast<std::uint32_t *>(nullptr), vertex_count, stream);
        },
        ...
        status = cub_query(
            ...
            [&](void *temporary, std::size_t &bytes) {
                return cub::DeviceRadixSort::SortKeys(
                    temporary, bytes, static_cast<std::uint64_t *>(nullptr),
                    static_cast<std::uint64_t *>(nullptr), edge_record_count, 0, 64, stream);
            },
    ...
    {
        StageRange range{"parallel-mater/validation"};
    ...
    face_normals_kernel<<<(triangle_count + block_size - 1) / block_size, block_size, 0, stream>>>(
        mesh.positions, mesh.triangles, output.face_normals_, area_normals, triangle_count,
        counters + 1);
    make_corner_records_kernel<<<(triangle_count + block_size - 1) / block_size, block_size, 0,
                                 stream>>>(mesh.triangles, corner_keys_a, corner_values_a,
                                           triangle_count);
    error = cudaPeekAtLastError();
    if (error != cudaSuccess) return cuda_status(error, "normal setup kernel launch failed");

    std::size_t bytes = cub_bytes;
    error = cub::DeviceRadixSort::SortPairs(cub_temporary, bytes, corner_keys_a, corner_keys_b,
                                            corner_values_a, corner_values_b, corner_count, 0, 64,
                                            stream);
    ...
    count_vertices_from_corner_keys_kernel<<<(corner_count + block_size - 1) / block_size,
                                             block_size, 0, stream>>>(corner_keys_b, corner_count,
                                                                      corner_counts);
    bytes = cub_bytes;
    error = cub::DeviceScan::ExclusiveSum(cub_temporary, bytes, corner_counts, corner_offsets,
                                          vertex_count, stream);
    if (error != cudaSuccess) return cuda_status(error, "corner prefix scan failed");
    error = cudaMemcpyAsync(corner_offsets + vertex_count, &corner_count, sizeof(corner_count),
    ...
    if (edge_record_count > 0) {
        make_edge_records_kernel<<<(edge_count + block_size - 1) / block_size, block_size, 0,
                                   stream>>>(sharp_edges.edges, edge_keys_a, edge_count);
        bytes = cub_bytes;
        error = cub::DeviceRadixSort::SortKeys(cub_temporary, bytes, edge_keys_a, edge_keys_b,
                                               edge_record_count, 0, 64, stream);
        if (error != cudaSuccess) return cuda_status(error, "sharp-edge sort failed");
        count_vertices_from_edge_keys_kernel<<<(edge_record_count + block_size - 1) / block_size,
                                               block_size, 0, stream>>>(
            edge_keys_b, edge_record_count, edge_counts);
    }
    bytes = cub_bytes;
    error = cub::DeviceScan::ExclusiveSum(cub_temporary, bytes, edge_counts, edge_offsets,
                                          vertex_count, stream);
    if (error != cudaSuccess) return cuda_status(error, "edge prefix scan failed");
    error = cudaMemcpyAsync(edge_offsets + vertex_count, &edge_record_count,
                            sizeof(edge_record_count), cudaMemcpyHostToDevice, stream);
    if (error != cudaSuccess) return cuda_status(error, "failed to terminate edge offsets");

    group_vertex_fans_kernel<<<(vertex_count + block_size - 1) / block_size, block_size, 0,
                               stream>>>(mesh.triangles, corner_values_b, corner_offsets,
                                         sorted_edge_keys, edge_offsets, labels, group_counts,
                                         vertex_count);
    bytes = cub_bytes;
    error = cub::DeviceScan::ExclusiveSum(cub_temporary, bytes, group_counts, group_offsets,
                                          vertex_count, stream);
    ...
    error = cudaMemcpyAsync(&host_last_offset, group_offsets + vertex_count - 1U,
                            sizeof(host_last_offset), cudaMemcpyDeviceToHost, stream);
    if (error == cudaSuccess) {
        error = cudaMemcpyAsync(&host_last_count, group_counts + vertex_count - 1U,
                                sizeof(host_last_count), cudaMemcpyDeviceToHost, stream);
    }
    if (error == cudaSuccess) {
        error = cudaMemcpyAsync(&host_degenerate_count, counters + 1, sizeof(host_degenerate_count),
    ...
    emit_vertex_normals_kernel<<<(vertex_count + block_size - 1) / block_size, block_size, 0,
                                 stream>>>(corner_values_b, corner_offsets, labels, group_offsets,
                                           area_normals, output.vertex_normals_,
                                           output.corner_normal_indices_, vertex_count);
...
Status build_hierarchy(DeviceMeshView mesh, HierarchyOptions options, Workspace &workspace,
    ...
    if (!status) return status;
    status =
        ensure_allocation(output.primitive_indices_, output.primitive_capacity_, triangle_count);
    if (!status) return status;
    ...
    status = cub_query(
        ...
        [&](void *temporary, std::size_t &bytes) {
            return cub::DeviceRadixSort::SortPairs(
                temporary, bytes, static_cast<std::uint64_t *>(nullptr),
                static_cast<std::uint64_t *>(nullptr), static_cast<std::uint32_t *>(nullptr),
                static_cast<std::uint32_t *>(nullptr), triangle_count, 0, 64, stream);
        },
    ...
    status = cub_query(
        ...
        [&](void *temporary, std::size_t &bytes) {
            return cub::DeviceRunLengthEncode::Encode(
                temporary, bytes, static_cast<std::uint64_t *>(nullptr),
                static_cast<std::uint64_t *>(nullptr), static_cast<std::uint32_t *>(nullptr),
                static_cast<std::uint32_t *>(nullptr), triangle_count, stream);
        },
    ...
    status = cub_query(
        ...
        [&](void *temporary, std::size_t &bytes) {
            return cub::DeviceSegmentedReduce::Reduce(
                temporary, bytes, static_cast<SegmentStats *>(nullptr),
                static_cast<SegmentStats *>(nullptr), triangle_count,
                static_cast<std::uint32_t *>(nullptr), static_cast<std::uint32_t *>(nullptr),
                MergeSegmentStats{}, segment_identity, stream);
        },
        "failed to size segmented reduction");
    if (!status) return status;

    ArenaLayout sizing;
    sizing.take<std::uint32_t>(
        4); // validation, run count, three totals (overlap intentional below)
    sizing.take<Vec3>(triangle_count);
    ...
    {
        StageRange range{"parallel-mater/centroid_generation"};
        hierarchy_geometry_kernel<<<(triangle_count + block_size - 1) / block_size, block_size, 0,
                                    stream>>>(mesh.positions, mesh.triangles, centroids,
                                              triangle_bounds, triangle_count);
    ...
    std::uint32_t zero = 0;
    cudaError_t error =
        cudaMemcpyAsync(offsets_a, &zero, sizeof(zero), cudaMemcpyHostToDevice, stream);
    if (error == cudaSuccess) {
        error = cudaMemcpyAsync(offsets_a + 1, &triangle_count, sizeof(triangle_count),
                                cudaMemcpyHostToDevice, stream);
    }
    if (error == cudaSuccess) {
    ...
    if (error == cudaSuccess) {
        error = cudaMemcpyAsync(all_branch_node_ids, &zero, sizeof(zero), cudaMemcpyHostToDevice,
                                stream);
    }
    if (error != cudaSuccess) return cuda_status(error, "failed to initialize hierarchy");

    if (triangle_count <= options.max_leaf_size) {
        error = cudaMemcpyAsync(output.primitive_indices_, primitives_a,
                                sizeof(std::uint32_t) * triangle_count, cudaMemcpyDeviceToDevice,
                                stream);
    ...
    while (segment_count > 0) {
        std::size_t bytes = cub_bytes;
        {
            StageRange range{"parallel-mater/reductions"};
            gather_segment_stats_kernel<<<(active_count + block_size - 1) / block_size, block_size,
                                          0, stream>>>(centroids, primitives_a, ordered_stats,
                                                       active_count);
            error = cub::DeviceSegmentedReduce::Reduce(
                cub_temporary, bytes, ordered_stats, segment_stats, segment_count, current_offsets,
                current_offsets + 1, MergeSegmentStats{}, segment_identity, stream);
            if (error != cudaSuccess) {
        ...
        {
            StageRange range{"parallel-mater/partitioning"};
            make_partition_keys_kernel<<<(active_count + block_size - 1) / block_size, block_size,
                                         0, stream>>>(centroids, primitives_a, segment_ids,
                                                      current_offsets, segment_stats, keys_a,
                                                      active_count, depth >= 32U);
            bytes = cub_bytes;
            error =
                cub::DeviceRadixSort::SortPairs(cub_temporary, bytes, keys_a, keys_b, primitives_a,
                                                primitives_b, active_count, 0, 64, stream);
            if (error != cudaSuccess) {
                return cuda_status(error, "hierarchy partition sort failed");
            }
            bytes = cub_bytes;
            error = cub::DeviceRunLengthEncode::Encode(cub_temporary, bytes, keys_b, unique_keys,
                                                       child_counts, device_run_count, active_count,
                                                       stream);
            if (error != cudaSuccess) {
                return cuda_status(error, "hierarchy child encoding failed");
            }
            error = cudaMemcpyAsync(&child_count, device_run_count, sizeof(child_count),
                                    cudaMemcpyDeviceToHost, stream);
            if (error != cudaSuccess) {
        ...
        if (child_count == 0 || node_count + child_count > maximum_nodes) {
            return {StatusCode::internal_error, cudaSuccess,
                    "hierarchy node capacity invariant failed"};
        }

        std::uint32_t host_totals[3]{};
        {
            StageRange range{"parallel-mater/scans"};
            classify_children_kernel<<<(child_count + block_size - 1) / block_size, block_size, 0,
                                       stream>>>(child_counts, branch_flags, branch_weights,
                                                 leaf_weights, child_count, options.max_leaf_size);
            error =
                cudaMemsetAsync(parent_counts, 0, sizeof(std::uint32_t) * segment_count, stream);
            if (error != cudaSuccess) return cuda_status(error, "failed to clear parent counts");
            count_parent_children_kernel<<<(child_count + block_size - 1) / block_size, block_size,
                                           0, stream>>>(unique_keys, child_count, parent_counts);

            auto scan = [&](const std::uint32_t *input, std::uint32_t *destination,
                            std::uint32_t count) {
                std::size_t scan_bytes = cub_bytes;
                return cub::DeviceScan::ExclusiveSum(cub_temporary, scan_bytes, input, destination,
                                                     count, stream);
            };
            ...
            collect_totals_kernel<<<1, 1, 0, stream>>>(
                branch_flags, branch_prefix, branch_weights, branch_weight_prefix, leaf_weights,
                leaf_weight_prefix, child_count, device_totals);
            error = cudaMemcpyAsync(host_totals, device_totals, sizeof(host_totals),
                                    cudaMemcpyDeviceToHost, stream);
        ...
        {
            StageRange range{"parallel-mater/packing"};
            set_parent_nodes_kernel<<<(segment_count + block_size - 1) / block_size, block_size, 0,
                                      stream>>>(current_node_ids, parent_counts, parent_prefix,
                                                output.nodes_, segment_count, node_count);
            initialize_child_nodes_kernel<<<(child_count + block_size - 1) / block_size, block_size,
                                            0, stream>>>(
                child_counts, branch_flags, branch_prefix, branch_weight_prefix, leaf_weight_prefix,
                next_offsets, next_node_ids, all_branch_node_ids, output.nodes_, child_count,
                node_count, branch_count_total, leaf_primitive_cursor);
            scatter_child_primitives_kernel<<<child_count, 128, 0, stream>>>(
                child_offsets, child_counts, branch_flags, branch_prefix, branch_weight_prefix,
                leaf_weight_prefix, primitives_b, primitives_a, segment_ids,
                output.primitive_indices_, child_count, leaf_primitive_cursor);
            leaf_bounds_kernel<<<(child_count + block_size - 1) / block_size, block_size, 0,
                                 stream>>>(child_offsets, child_counts, branch_flags, primitives_b,
                                           triangle_bounds, output.nodes_, child_count, node_count);

            error = cudaMemcpyAsync(next_offsets + next_branch_count, &next_active_count,
                                    sizeof(next_active_count), cudaMemcpyHostToDevice, stream);
            if (error != cudaSuccess) {
    ...
    status = ensure_allocation(output.branch_node_ids_, output.branch_node_capacity_,
                               branch_count_total);
    if (!status) return status;
    error = cudaMemcpyAsync(output.branch_node_ids_, all_branch_node_ids,
    ...
    {
        StageRange range{"parallel-mater/bounds_propagation"};
        for (auto level = branch_levels.rbegin(); level != branch_levels.rend(); ++level) {
            branch_bounds_kernel<<<(level->second + block_size - 1) / block_size, block_size, 0,
                                   stream>>>(all_branch_node_ids + level->first, output.nodes_,
                                             level->second);
...
Status refit_hierarchy(DeviceAabbView primitives, Hierarchy &hierarchy, cudaStream_t stream) {
    ...
    if (primitives.primitive_count != hierarchy.primitive_count_ || hierarchy.nodes_ == nullptr ||
        hierarchy.primitive_indices_ == nullptr || hierarchy.statistics_.node_count == 0U) {
    ...
    if (hierarchy.proxy_validation_ == nullptr) {
        const cudaError_t allocation =
            cudaMalloc(&hierarchy.proxy_validation_, sizeof(std::uint32_t));
        if (allocation != cudaSuccess) {
            return cuda_status(allocation, "refit validation allocation failed");
        }
    }
    cudaError_t error =
        cudaMemsetAsync(hierarchy.proxy_validation_, 0, sizeof(std::uint32_t), stream);
    ...
    validate_aabb_kernel<<<(primitive_count + block_size - 1U) / block_size, block_size, 0,
                           stream>>>(primitives.bounds, primitive_count,
                                     hierarchy.proxy_validation_);
    std::uint32_t invalid_bounds = 0U;
    error = cudaMemcpyAsync(&invalid_bounds, hierarchy.proxy_validation_, sizeof(invalid_bounds),
                            cudaMemcpyDeviceToHost, stream);
    ...
    refit_leaf_bounds_kernel<<<(hierarchy.statistics_.node_count + block_size - 1U) / block_size,
                               block_size, 0, stream>>>(
        primitives.bounds, hierarchy.primitive_indices_, hierarchy.nodes_,
        hierarchy.statistics_.node_count);
    for (auto level = hierarchy.branch_levels_.rbegin(); level != hierarchy.branch_levels_.rend();
         ++level) {
        branch_bounds_kernel<<<(level->second + block_size - 1U) / block_size, block_size, 0,
                               stream>>>(hierarchy.branch_node_ids_ + level->first,
...
Status build_hierarchy(DeviceAabbView primitives, HierarchyOptions options, Workspace &workspace,
    ...
    const auto primitive_count = static_cast<std::uint32_t>(primitives.primitive_count);
    Status status = ensure_allocation(output.proxy_positions_, output.proxy_position_capacity_,
                                      static_cast<std::size_t>(primitive_count) * 3U);
    if (!status) return status;
    status = ensure_allocation(output.proxy_triangles_, output.proxy_triangle_capacity_,
                               primitive_count);
    if (!status) return status;
    ...
    cudaError_t error = cudaMemsetAsync(output.proxy_validation_, 0, sizeof(std::uint32_t), stream);
    if (error != cudaSuccess) return cuda_status(error, "failed to clear AABB validation");
    aabb_proxy_kernel<<<(primitive_count + block_size - 1U) / block_size, block_size, 0, stream>>>(
        primitives.bounds, output.proxy_positions_, output.proxy_triangles_, primitive_count,
        output.proxy_validation_);
    std::uint32_t invalid_bounds = 0;
    error = cudaMemcpyAsync(&invalid_bounds, output.proxy_validation_, sizeof(invalid_bounds),
                            cudaMemcpyDeviceToHost, stream);
    ...
    return build_hierarchy(DeviceMeshView{output.proxy_positions_,
                                          static_cast<std::uint64_t>(primitive_count) * 3U,
                                          output.proxy_triangles_, primitive_count},
                           options, workspace, output, stream);
}
