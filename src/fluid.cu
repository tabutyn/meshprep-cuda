...
[[nodiscard]] bool valid(FluidOptions options) noexcept {
    return finite(options.particle_radius) && options.particle_radius > 0.0F &&
           ...
           finite(options.repulsion) && options.repulsion >= 0.0F && finite(options.viscosity) &&
           options.viscosity >= 0.0F && finite(options.velocity_damping) &&
           options.velocity_damping >= 0.0F && finite(options.maximum_speed) &&
...
__device__ std::uint32_t lower_bound(const std::uint64_t *keys, std::uint32_t count,
                                     std::uint64_t key) {
...
__global__ void emit_cells(const float3 *positions, std::uint64_t *keys, std::uint32_t *indices,
...
__global__ void compute_forces(const float3 *positions, const float3 *velocities,
    ...
    for (int dz = -1; dz <= 1; ++dz) {
        for (int dy = -1; dy <= 1; ++dy) {
            for (int dx = -1; dx <= 1; ++dx) {
                const std::uint64_t key = cell_key(cell_x + dx, cell_y + dy, cell_z + dz);
                const std::uint32_t first = lower_bound(keys, count, key);
                for (std::uint32_t item = first; item < count && keys[item] == key; ++item) {
                    ...
                    if (!(distance_squared > 1.0e-12F) ||
                        distance_squared >= options.interaction_radius * options.interaction_radius)
                        continue;
                    ++neighbors;
                    const float distance = sqrtf(distance_squared);
                    const float q = 1.0F - distance * inverse_radius;
                    const float3 normal = multiply(delta, 1.0F / distance);
                    force = add(force, multiply(normal, options.repulsion * q * q));
                    force = add(force, multiply(subtract(velocities[other], velocity),
                                                options.viscosity * q));
                }
            }
        }
...
__global__ void integrate(float3 *positions, float3 *velocities, const float3 *forces,
    ...
    velocity =
        add(velocity, multiply(add(substep.acceleration, multiply(forces[particle], inverse_mass)),
                               substep.timestep));
    velocity = add(velocity, multiply(external_impulses[particle], inverse_mass));
    velocity = multiply(velocity, expf(-options.velocity_damping * substep.timestep));
    const float speed_squared = dot(velocity, velocity);
    if (speed_squared > options.maximum_speed * options.maximum_speed) {
        velocity = multiply(velocity, options.maximum_speed / sqrtf(speed_squared));
    }
    const float3 position = add(positions[particle], multiply(velocity, substep.timestep));
...
template <typename T> Status allocate(T *&pointer, std::size_t count) noexcept {
    if (count == 0U) return {};
    return cuda_status(cudaMalloc(&pointer, count * sizeof(T)), "could not allocate fluid storage");
}
...
Status Fluid::create(std::span<const FluidParticle> particles, FluidOptions options, Fluid &output,
                     cudaStream_t stream) noexcept {
    return output.initialize(particles, options, stream);
}

Status Fluid::initialize(std::span<const FluidParticle> particles, FluidOptions options,
                         cudaStream_t stream) noexcept {
    if (particles.empty() ||
    ...
    if (!(status = cuda_status(cudaMemcpyAsync(replacement->positions, host_positions.get(), bytes,
        ...
        !(status = cuda_status(cudaMemcpyAsync(replacement->velocities, host_velocities.get(),
                                               bytes, cudaMemcpyHostToDevice, stream),
                               "could not upload fluid velocities")) ||
        !(status = cuda_status(cudaMemcpyAsync(replacement->initial_positions, host_positions.get(),
                                               bytes, cudaMemcpyHostToDevice, stream),
    ...
    if (!(status = cuda_status(cub::DeviceRadixSort::SortPairs(
                                   nullptr, replacement->cub_storage_bytes, replacement->keys_a,
                                   replacement->keys_b, replacement->indices_a,
                                   replacement->indices_b, static_cast<int>(count), 0, 64, stream),
                               "could not size fluid sort workspace")))
        return status;
    if (!(status =
              cuda_status(cudaMalloc(&replacement->cub_storage, replacement->cub_storage_bytes),
                          "could not allocate fluid sort workspace")))
        return status;
    if (!(status = cuda_status(
              cudaMemsetAsync(replacement->diagnostics, 0, 2U * sizeof(std::uint32_t), stream),
              "could not clear fluid diagnostics")) ||
        !(status = cuda_status(cudaStreamSynchronize(stream),
                               "could not complete fluid initialization")))
        return status;
...
Status Fluid::begin_frame(FrameOptions frame, cudaStream_t stream) noexcept {
    ...
    Status status =
        cuda_status(cudaMemsetAsync(impl_->diagnostics, 0, 2U * sizeof(std::uint32_t), stream),
                    "could not clear fluid diagnostics");
    if (status) {
        impl_->frame = frame;
...
Status Fluid::prepare_substep(SubstepContext substep, cudaStream_t stream) noexcept {
    ...
    if (substep.index != impl_->next_substep || substep.count != impl_->frame.substeps ||
        !finite(substep.timestep) || substep.timestep <= 0.0F || !finite(substep.acceleration))
        return invalid("invalid fluid substep");
    const std::uint32_t blocks = (impl_->count + block_size - 1U) / block_size;
    emit_cells<<<blocks, block_size, 0, stream>>>(impl_->positions, impl_->keys_a, impl_->indices_a,
                                                  impl_->count,
                                                  1.0F / impl_->options.interaction_radius);
    Status status = cuda_status(cudaGetLastError(), "could not emit fluid cells");
    if (!status) return status;
    status = cuda_status(
        cub::DeviceRadixSort::SortPairs(impl_->cub_storage, impl_->cub_storage_bytes, impl_->keys_a,
                                        impl_->keys_b, impl_->indices_a, impl_->indices_b,
                                        static_cast<int>(impl_->count), 0, 64, stream),
        "could not sort fluid cells");
    if (!status) return status;
    status = cuda_status(cudaMemsetAsync(impl_->external_impulses, 0,
                                         static_cast<std::size_t>(impl_->count) * sizeof(float3),
                                         stream),
                         "could not clear fluid coupling impulses");
    if (!status) return status;
    compute_forces<<<blocks, block_size, 0, stream>>>(
        impl_->positions, impl_->velocities, impl_->keys_b, impl_->indices_b, impl_->count,
...
Status Fluid::finish_substep(SubstepContext substep, cudaStream_t stream) noexcept {
    if (!impl_ || !impl_->frame_active) return invalid("fluid frame is not active");
    if (substep.index != impl_->next_substep || substep.count != impl_->frame.substeps)
        return invalid("fluid substep is out of order");
    const std::uint32_t blocks = (impl_->count + block_size - 1U) / block_size;
    integrate<<<blocks, block_size, 0, stream>>>(impl_->positions, impl_->velocities, impl_->forces,
                                                 impl_->external_impulses, impl_->count,
                                                 impl_->options, substep, impl_->diagnostics);
...
Status Fluid::collect_statistics_async(Completion &completion, cudaStream_t stream) noexcept {
    ...
    if (completion.pending()) return invalid("completion token is already pending");
    Status status =
        cuda_status(cudaMemcpyAsync(impl_->host_diagnostics, impl_->diagnostics,
                                    2U * sizeof(std::uint32_t), cudaMemcpyDeviceToHost, stream),
                    "could not download fluid diagnostics");
...
Status Fluid::advance_async(FrameOptions frame, Completion &completion,
                            cudaStream_t stream) noexcept {
...
Status Fluid::reset(cudaStream_t stream) noexcept {
    ...
    Status status = cuda_status(cudaMemcpyAsync(impl_->positions, impl_->initial_positions, bytes,
                                                cudaMemcpyDeviceToDevice, stream),
                                "could not reset fluid positions");
    if (!status) return status;
    status = cuda_status(cudaMemcpyAsync(impl_->velocities, impl_->initial_velocities, bytes,
                                         cudaMemcpyDeviceToDevice, stream),
...
FluidOptions Fluid::options() const noexcept { return impl_ ? impl_->options : FluidOptions{}; }
FluidView Fluid::particles() const noexcept {
    return impl_ ? FluidView{impl_->positions,
                             impl_->velocities,
