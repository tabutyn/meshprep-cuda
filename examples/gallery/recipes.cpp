...
RecipeConfigError validate_recipe_config(RecipeConfig config) noexcept {
    ...
    if (config.substep_count && (*config.substep_count < 1U || *config.substep_count > 16U))
        return RecipeConfigError::invalid_substeps;
...
std::string_view recipe_config_error_message(RecipeConfigError error) noexcept {
    ...
    case RecipeConfigError::invalid_substeps:
        return "simulation substeps must be between 1 and 16";
