...
int main() {
    ...
    for (std::uint32_t z = 0U; z < 4U; ++z) {
        for (std::uint32_t y = 0U; y < 4U; ++y) {
            for (std::uint32_t x = 0U; x < 4U; ++x) {
                initial_particles.push_back({make_float3(0.05F * static_cast<float>(x),
    ...
    std::printf("fluid=%u cloth=%u rope=%u sphere_y=%.3f\n", fluid.statistics().particle_count,
                cloth.nodes().node_count, rope.nodes().node_count, sphere.state().position.y);
    return 0;
}
