...
int main() {
    ...
    const parallel_mater::DeviceMeshView mesh{device_positions, 3, device_triangles, 1};
    ...
    if (!normal_status || !hierarchy_status) {
        std::fprintf(stderr, "ParallelMater failed: %s / %s\n", normal_status.message,
                     hierarchy_status.message);
        cudaFree(device_triangles);
        cudaFree(device_positions);
        return 1;
    }
    std::printf("face normals: 1, vertex normals: %u, hierarchy nodes: %u\n",
                normals.statistics().vertex_normal_count, hierarchy.statistics().node_count);
    cudaFree(device_triangles);
    cudaFree(device_positions);
    return 0;
}
