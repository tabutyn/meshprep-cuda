...
int main() {
    ...
    status = colliders.update(std::span<const parallel_mater::physics::Collider>(&obstacle, 1U));
    ...
    std::printf("particles=%u frames=%llu advection_ms=%.3f respawns=%llu\n", particles.count,
                static_cast<unsigned long long>(statistics.frame_index), timings.integrate_ms,
