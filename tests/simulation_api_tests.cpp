...
constexpr std::uint32_t bits(Component value) noexcept { return static_cast<std::uint32_t>(value); }
...
void test_recipe_catalog() {
    ...
    expect(softbody_rope != nullptr && softbody_rope->recipe == SimulationRecipe::soft_body_rope,
    ...
    expect(parallel_mater::examples::has_component(Component::cloth | Component::soft_body,
                                                   Component::cloth),
...
void test_fixed_step_contract() {
    ...
    expect(!parallel_mater::examples::valid({std::numeric_limits<float>::quiet_NaN()}),
           "non-finite fixed timestep must be rejected by a solver boundary");
    expect(!parallel_mater::examples::valid({std::numeric_limits<float>::infinity()}),
...
void test_owning_simulation_contract() {
    ...
    expect(parallel_mater::examples::requires_soft_body_asset(SimulationRecipe::water_soft_body),
           "water-softbody composition must request a public soft-body asset");
...
void test_borrowed_render_views() {
    ...
    const parallel_mater::examples::ParticleRenderView particles{positions, velocities, 3U, 0.025F};
    const parallel_mater::examples::SurfaceRenderView surface{
        {positions, 3U, triangles, 1U}, normals, texcoords, triangle_active};
    const parallel_mater::examples::RigidBodyRenderView rigid{{positions, 3U, triangles, 1U},
                                                              {1.0F, 2.0F, 3.0F},
                                                              {0.0F, 0.0F, 0.0F, 1.0F},
                                                              {2.0F, 3.0F, 4.0F}};
    ...
    const parallel_mater::examples::FrameRenderView frame{
        particle_systems.data(), 1U, surfaces.data(), 1U,
        rigid_bodies.data(),     1U, lattices.data(), 1U};

    expect(frame.particle_systems == particle_systems.data(),
           "frame must borrow the particle-view array");
    expect(frame.surfaces == surfaces.data(), "frame must borrow the surface-view array");
    ...
    expect(frame.lattices == lattices.data() && frame.lattice_count == 1U &&
               frame.lattices[0].positions == positions && frame.lattices[0].flags == flags &&
               frame.lattices[0].bonds == bonds && frame.lattices[0].bond_active == bond_active,
    ...
    expect(frame.rigid_bodies[0].translation.x == 1.0F && frame.rigid_bodies[0].scale.z == 4.0F &&
