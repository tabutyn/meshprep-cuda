...
int main()
{
    ...
    try {
        ...
        require(statistics.frame_index==180U && statistics.respawn_count>0U &&
                ...
                statistics.maximum_speed>0.1F && timing.integrate_ms>0.0F,
        ...
        for (const float3 point:positions) {
            finite_count+=std::isfinite(point.x)&&std::isfinite(point.y)&&
        ...
        require(std::isfinite(impulse.x) && std::isfinite(impulse.y) &&
